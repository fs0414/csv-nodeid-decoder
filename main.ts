...
export class CsvBase64Decoder {
  ...
  async process(): Promise<void> {
    try {
      ...
      if (!fileInfo.isFile) {
        throw new Error(
          `指定されたパスはファイルではありません: ${this.options.filePath}`,
        );
      }
        ...
        targetColumnIndices.forEach((columnName, index) => {
          if (index < row.length && row[index]) {
            ...
            if (decodedValue !== null) {
              ...
              console.warn(
                `行 ${i + 1}, カラム "${columnName}" のデコードに失敗しました`,
              );
              // デコード失敗時は元の値を保持
            }
          }
      ...
      console.log(
        `処理したカラム: ${
          Array.from(targetColumnIndices.values()).join(", ")
        }`,
      );
...
if (import.meta.main) {
  ...
  if (args.length < 2) {
    console.error(
      "使用方法: deno run --allow-read --allow-write csv-base64-decoder.ts <CSVファイルパス> <カラム名1> [カラム名2] ...",
    );
    Deno.exit(1);
  }
